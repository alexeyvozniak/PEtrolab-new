...
function SourcePane({ items, assignments, activePath, setActivePath, phase }) {
  const moveFocus = (event, index) => {
    ...
    const nextIndex = key === 'Home' ? 0 : key === 'End' ? items.length - 1 : Math.max(0, Math.min(items.length - 1, index + (key === 'ArrowDown' ? 1 : -1)));
    const nextPath = items[nextIndex]?.source_path;
    if (!nextPath) return;
  ...
  return <aside className="image-source-pane">
    ...
    <div className="image-source-list">
      {items.map((item, index) => {
        ...
        return <button aria-current={focused ? "true" : undefined} className={focused ? "focused" : ""} type="button" key={item.source_path} onClick={() => setActivePath(item.source_path)} onKeyDown={(event) => moveFocus(event, index)}>
          <span className="image-source-icon"><FileImage size={24} weight="duotone" /></span>
          <span><b title={item.source_path}>{index + 1}. {item.display_name}</b><small>{parentFolder(item.source_path) ? `${parentFolder(item.source_path)} · ` : ""}{item.width_px} × {item.height_px} px · {(item.format || item.mime_type?.split("/").pop() || "image").toUpperCase()}</small><em>{phase === "assignment" ? (assignmentReady(assignment) ? "назначено" : "нужно подтвердить") : placementCount > 0 ? `${placementCount} размещено` : "Без точек"}</em></span>
        </button>;
      })}
    </div>
...
function AssignmentStep({ items, inspection, assignments, setAssignments, selected, setSelected, activeItem, activeAssignment, setActivePath, busy, lastExcludedCount, onExcludeSelected, onRestoreExcluded, onCancel, onInvalidatePlan, onContinue }) {
  const [listFilter, setListFilter] = useState("all");
  const duplicatePaths = useMemo(() => {
    const activePaths = new Set(items.map((item) => item.source_path));
    return new Set((inspection?.duplicate_groups || []).flat().filter((path) => activePaths.has(path)));
  }, [inspection, items]);
  ...
  const selectedDuplicateItems = selectedItems.filter((item) => duplicatePaths.has(item.source_path));
  ...
  const wouldEmptyBatch = selectedDuplicateItems.length === items.length;
  const showBulkActions = selectedItems.length >= 2;
  const visibleItems = items.filter((item) => (
    listFilter === "pending" ? !assignmentReady(assignments[item.source_path])
      : listFilter === "duplicates" ? duplicatePaths.has(item.source_path)
        : true
  ));
  const moveActiveRow = (event, index) => {
    const key = event.key;
    if (['Enter', ' '].includes(key)) {
      event.preventDefault();
      setActivePath(visibleItems[index]?.source_path || "");
      return;
    }
    if (!['ArrowDown', 'ArrowUp', 'Home', 'End'].includes(key)) return;
    event.preventDefault();
    const nextIndex = key === 'Home' ? 0 : key === 'End' ? visibleItems.length - 1 : Math.max(0, Math.min(visibleItems.length - 1, index + (key === 'ArrowDown' ? 1 : -1)));
    const nextPath = visibleItems[nextIndex]?.source_path;
    if (!nextPath) return;
    setActivePath(nextPath);
    event.currentTarget.parentElement?.querySelectorAll('[data-image-assignment-row]')[nextIndex]?.focus();
  };

  const patchAssignment = (path, values) => {
    onInvalidatePlan();
    setAssignments((current) => ({ ...current, [path]: { ...current[path], ...values, confirmed: false } }));
  };
  ...
  const applyActiveToSelection = () => {
    ...
    setAssignments((current) => {
      const next = { ...current };
      selectedItems.forEach((item) => { next[item.source_path] = { ...next[item.source_path], ...shared, confirmed: true }; });
      return next;
    });
  };
  const confirmActiveAssignment = () => {
    if (!activeItem || !activeAssignment) return;
    const values = {
      media_type: activeAssignment.media_type.trim(),
      sample_name: activeAssignment.sample_name.trim(),
      thin_section_name: activeAssignment.thin_section_name.trim(),
    };
    if (!values.media_type || !values.sample_name || !values.thin_section_name) return;
    onInvalidatePlan();
    setAssignments((current) => ({
      ...current,
      [activeItem.source_path]: { ...current[activeItem.source_path], ...values, confirmed: true },
    }));
  };
  const activeAssignmentComplete = Boolean(
    activeAssignment?.media_type.trim() && activeAssignment?.sample_name.trim() && activeAssignment?.thin_section_name.trim(),
  );
  const continueHint = duplicatePaths.size > 0
    ? `${duplicatePaths.size} дубликатов нужно исключить из пакета`
    : pendingCount > 0 ? `${pendingCount} ${pendingCount === 1 ? "назначение требует" : "назначения требуют"} подтверждения`
      : "Назначения проверены";
  const pendingSummary = pendingCount > 0
    ? `${pendingCount} ${pendingCount === 1 ? "назначение требует" : "назначения требуют"} подтверждения`
    : "назначения проверены";

  return <>
    <main className="image-assignment-pane">
      <header>
        <div><h2>Назначь Sample и шлиф <span aria-hidden="true">{items.length}</span></h2><p>Выбери строку, чтобы проверить её назначение.</p></div>
      </header>
      <div className="image-batch-summary" aria-label="Сводка пакета изображений" aria-live="polite">
        <button type="button" className={listFilter === "all" ? "active" : ""} onClick={() => setListFilter("all")}><strong>{items.length}</strong><span>в пакете</span></button>
        <button type="button" className={readyCount ? "positive" : "muted"} onClick={() => setListFilter("all")}><strong>{readyCount}</strong><span>готовы</span></button>
        <button type="button" className={`${pendingCount ? "attention" : "positive"}${listFilter === "pending" ? " active" : ""}`} onClick={() => setListFilter(listFilter === "pending" ? "all" : "pending")}><strong>{pendingCount}</strong><span>требуют проверки</span></button>
        <button type="button" className={`${duplicatePaths.size ? "danger" : "positive"}${listFilter === "duplicates" ? " active" : ""}`} onClick={() => setListFilter(listFilter === "duplicates" ? "all" : "duplicates")} disabled={!duplicatePaths.size}><strong>{duplicatePaths.size}</strong><span>дубликатов</span></button>
      </div>
      {showBulkActions && <div className="image-bulk-toolbar">
      ...
      {lastExcludedCount > 0 && <div className="image-batch-undo" role="status"><Info size={17} /><span>Исключено из временного пакета: {lastExcludedCount}. Исходные файлы не изменены.</span><button type="button" onClick={onRestoreExcluded} disabled={busy}>Вернуть в пакет</button></div>}
      {duplicatePaths.size > 0 && <div className="image-duplicate-warning"><Warning size={18} /><span>Одинаковые физические изображения не будут импортированы дважды. Выбери строки, которые нужно исключить из текущего пакета.</span><button className="danger-outline" type="button" onClick={() => onExcludeSelected(selectedDuplicateItems.map((item) => item.source_path))} disabled={busy || selectedDuplicateItems.length === 0 || wouldEmptyBatch} title={wouldEmptyBatch ? "Оставь в пакете хотя бы один файл." : undefined}>Исключить выбранные дубликаты ({selectedDuplicateItems.length})</button>{wouldEmptyBatch && <small className="image-duplicate-preserve-note">Оставь хотя бы один файл в пакете.</small>}</div>}
      <div className="image-assignment-table" role="table" aria-label="Назначения импортируемых изображений">
        ...
        {visibleItems.map((item, index) => {
          const assignment = assignments[item.source_path];
          return <div aria-current={item.source_path === activeItem?.source_path ? "true" : undefined} className={`${item.source_path === activeItem?.source_path ? "active " : ""}${duplicatePaths.has(item.source_path) ? "duplicate" : ""}`} data-image-assignment-row role="row" tabIndex={0} key={item.source_path} onClick={() => setActivePath(item.source_path)} onKeyDown={(event) => moveActiveRow(event, index)}>
        ...
        {!visibleItems.length && <div className="image-assignment-empty" role="status">В этой категории нет изображений.</div>}
      </div>
    </main>
    <aside className="image-inspector">
      ...
      <section className="image-assignment-form">
        ...
        <label>Тип изображения<input aria-label="Тип изображения" list="image-media-type-options" value={activeAssignment?.media_type || ""} onChange={(event) => patchAssignment(activeItem.source_path, { media_type: event.target.value })} placeholder="BSE, PPL, XPL или свой тип" /><datalist id="image-media-type-options">{MEDIA_TYPES.map((type) => <option value={type} key={type} />)}</datalist></label>
        ...
        <button className="primary-button image-confirm-assignment" type="button" onClick={confirmActiveAssignment} disabled={busy || !activeAssignmentComplete}><CheckCircle size={17} /> Подтвердить это назначение</button>
        {showBulkActions && <button className="outline-button image-bulk-apply" type="button" onClick={applyActiveToSelection} disabled={busy || !activeAssignmentComplete}><Copy size={17} /> Применить к выбранным ({selected.size})</button>}
      </section>
    </aside>
    <footer className="image-commit-bar">
      <button className="outline-button" type="button" onClick={onCancel} disabled={busy}>Отмена</button><div><b>{readyCount} готовы</b><span>{` · ${pendingSummary}`}</span></div>
      <small id="image-assignment-continue-hint">{continueHint}. Исходные файлы не изменяются.</small>
      <button className="primary-button" type="button" aria-describedby="image-assignment-continue-hint" disabled={busy || !canContinue} onClick={onContinue}>Продолжить: точки <ArrowRight size={18} /></button>
    </footer>
  </>;
}
...
function ImageCanvas({ item, assignment, preview, previewLoading, previewError, onRetryPreview, zoom, geometryKind, draft, setDraft, selectedPoint, onSelectPlacement, interactive = true }) {
  ...
  const geometryFromDrag = (start, end) => {
    ...
    return {
      kind: geometryKind,
      x_px: end.x < start.x ? start.x - width : start.x,
      y_px: end.y < start.y ? start.y - height : start.y,
      width_px: width,
      height_px: height,
    };
  };
  ...
  const onPointerMove = (event) => {
    ...
    const geometry = geometryFromDrag(dragStart.current, coordinates(event));
    if (!geometry) setDraft(null);
    else pointDraft(selectedPoint, geometry);
  };
  const onPointerUp = (event) => {
    if (!selectedPoint || !dragStart.current || geometryKind === "point") return;
    const start = dragStart.current;
    dragStart.current = null;
    const geometry = geometryFromDrag(start, coordinates(event));
    if (!geometry) setDraft(null);
    else pointDraft(selectedPoint, geometry);
  };
  const onPointerCancel = () => { dragStart.current = null; setDraft(null); };
  ...
  useEffect(() => {
    if (interactive && selectedPoint && preview) canvasRef.current?.focus();
  }, [interactive, preview, selectedPoint]);
  return <div className="image-canvas-scroll">
    {previewLoading && <div className="image-preview-state">Готовлю безопасный preview…</div>}
    {previewError && <div className="image-preview-state error"><Warning size={20} /><span>{previewError}</span>{onRetryPreview && <button type="button" onClick={(event) => { event.stopPropagation(); onRetryPreview(); }} disabled={previewLoading}>Повторить preview</button>}</div>}
    {preview && <div
      ref={canvasRef}
...
function PlacementStep({ items, assignments, setAssignments, points, activeItem, activeAssignment, activePath, setActivePath, busy, previews, previewLoading, previewError, onInvalidatePlan, onBack, onPreview, onPlan, onOpenAnalyses }) {
  ...
  useEffect(() => {
    setSelectedPointId(activeAssignment?.placements?.[0]?.analytical_point_id || ""); setPointScope("same_sample"); setPointSearch("");
  ...
  const moveFileFocus = (event, index) => {
    const key = event.key;
    if (!['ArrowDown', 'ArrowUp', 'Home', 'End'].includes(key)) return;
    event.preventDefault();
    const nextIndex = key === 'Home' ? 0 : key === 'End' ? items.length - 1 : Math.max(0, Math.min(items.length - 1, index + (key === 'ArrowDown' ? 1 : -1)));
  ...
  return <>
      ...
      <div>
        {items.map((item, index) => {
          ...
          return <button
            ...
            onKeyDown={(event) => moveFileFocus(event, index)}
          >
    ...
    <section className="image-point-workspace">
      ...
      <main className="image-point-canvas-pane">
        ...
        <ImageCanvas item={activeItem} assignment={activeAssignment} preview={previews[activePath]} previewLoading={previewLoading.has(activePath)} previewError={previewError[activePath]} onRetryPreview={() => onPreview(activeItem, true)} zoom={zoom} geometryKind={geometryKind} draft={draft} setDraft={setDraft} selectedPoint={selectedPoint} onSelectPlacement={setSelectedPointId} />
      ...
      <aside className="image-point-inspector">
        ...
        {selectedPoint ? <>
          <section><h3>Связанные Analyses</h3><div className="point-analysis-list">{selectedPoint.analysis_ids.map((id) => { const member = (selectedPoint.analysis_members || []).find((item) => item.analysis_id === id); return <div key={id}><span>{member?.method || "Метод не указан"}</span><b title={id}>{shortenId(id)}</b></div>; })}</div></section>
...
function ReviewStep({ plan, assignments, activePath, setActivePath, previews, previewLoading, previewError, onPreview, busy, onBack, onApply }) {
  ...
  const moveFocus = (event, index) => {
    ...
    event.preventDefault();
    const nextIndex = event.key === 'Home' ? 0 : event.key === 'End' ? items.length - 1 : Math.max(0, Math.min(items.length - 1, index + (event.key === 'ArrowDown' ? 1 : -1)));
    const nextPath = items[nextIndex]?.source_path;
    if (!nextPath) return;
    setActivePath(nextPath);
    event.currentTarget.parentElement?.querySelectorAll('button')[nextIndex]?.focus();
    ...
    <main className="image-review-pane">
      ...
      <div className="image-review-table" role="table" aria-label="Проверка импорта изображений">
        ...
        {items.map((item) => {
          ...
          const placed = item.placements.length > 0;
          const active = item.source_path === activeItem.source_path;
          return <button aria-current={active ? "true" : undefined} type="button" role="row" className={active ? "active" : ""} key={item.source_path} onClick={() => setActivePath(item.source_path)} onKeyDown={(event) => moveFocus(event, items.indexOf(item))}><span>{item.display_name}</span><span>{item.sample_name}</span><span>{item.thin_section_name}</span><span className={`image-type type-${item.media_type}`}>{item.media_type}</span><span>{item.placements.length}</span><span className={exceptions ? "review-question" : ""}>{exceptions}</span><span className={placed ? "image-ready" : "image-unplaced"}>{placed ? <><CheckCircle size={15} weight="fill" /> Готово</> : <><Warning size={15} /> Без точек</>}</span></button>;
        })}
      </div>
      {unplacedMediaCount > 0 && <div className="image-review-warning" role="status"><Warning size={18} /><span><b>{unplacedMediaCount} изображений без пространственных точек.</b> Импорт всё равно разрешён, если это сделано намеренно.</span></div>}
...
export function ImagesWorkspace({ inspection, plan, points = { items: [] }, busy, onChoose, onChooseFolder, onPreview = async () => null, onPlan, onApply, onCancel, onInvalidatePlan = () => {}, onOpenAnalyses }) {
  ...
  const [lastExcludedPaths, setLastExcludedPaths] = useState([]);
  const [activePath, setActivePath] = useState("");
  const [previews, setPreviews] = useState({});
  const [previewLoading, setPreviewLoading] = useState(() => new Set());
  const [previewError, setPreviewError] = useState({});
  const requestedPreviews = useRef(new Set());
  const previewBatchRef = useRef(inspection);

  useEffect(() => {
    previewBatchRef.current = inspection;
    setExcludedPaths(new Set());
    setLastExcludedPaths([]);
    setAssignments(Object.fromEntries(inspectedItems.map((item) => [item.source_path, initialAssignment(item)])));
    setSelected(new Set(inspectedItems.map((item) => item.source_path)));
    setActivePath(inspectedItems[0]?.source_path || "");
    requestedPreviews.current.clear();
    setPreviews({}); setPreviewError({}); setPreviewLoading(new Set()); setPhase("assignment");
  }, [inspection]);
  useEffect(() => { if (plan) setPhase("review"); }, [plan]);

  const items = inspectedItems.filter((item) => !excludedPaths.has(item.source_path));
  const activeItem = items.find((item) => item.source_path === activePath) || items[0];
  const activeAssignment = activeItem ? assignments[activeItem.source_path] : null;
  const excludeSelected = (paths) => {
    const excluded = new Set(paths);
    if (!excluded.size) return;
    onInvalidatePlan();
    setExcludedPaths((current) => new Set([...current, ...excluded]));
    setLastExcludedPaths([...excluded]);
    setSelected((current) => new Set([...current].filter((path) => !excluded.has(path))));
    setActivePath((current) => excluded.has(current)
      ? items.find((item) => !excluded.has(item.source_path))?.source_path || ""
      : current);
  };
  const restoreLastExcluded = () => {
    if (!lastExcludedPaths.length) return;
    onInvalidatePlan();
    const restored = new Set(lastExcludedPaths);
    setExcludedPaths((current) => new Set([...current].filter((path) => !restored.has(path))));
    setLastExcludedPaths([]);
    setActivePath(lastExcludedPaths[0]);
  };
  const loadPreview = useCallback(async (item, force = false) => {
    if (!item || (!force && requestedPreviews.current.has(item.source_path))) return;
    const batch = previewBatchRef.current;
    if (force) {
      requestedPreviews.current.delete(item.source_path);
      setPreviews((current) => {
        const next = { ...current };
    ...
    setPreviewLoading((current) => new Set([...current, item.source_path]));
  ...
  return <section className={`images-workspace${phase === "points" ? " image-placement-workspace" : ""}${phase === "assignment" ? " image-assignment-workspace" : ""}`}>
    {phase === "assignment"
      ? <AssignmentStep items={items} inspection={inspection} assignments={assignments} setAssignments={setAssignments} selected={selected} setSelected={setSelected} activeItem={activeItem} activeAssignment={activeAssignment} setActivePath={setActivePath} busy={busy} lastExcludedCount={lastExcludedPaths.length} onExcludeSelected={excludeSelected} onRestoreExcluded={restoreLastExcluded} onCancel={onCancel} onInvalidatePlan={onInvalidatePlan} onContinue={() => setPhase("points")} />

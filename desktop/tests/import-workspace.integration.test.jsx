...
test('real formula path saves APFU, reopens history and invalidates a changed Fe preview', async () => {
  ...
  await user.click(screen.getByText('Формула минерала', { selector: 'summary' }));
...
test('real mica formula requires three choices, saves OH provenance and preserves choices after an error', async () => {
  ...
  await user.click(await enabledButton('Приняты пользователем 1'));
  await user.click(screen.getByText('Формула минерала', { selector: 'summary' }));
  const method = await screen.findByRole('combobox', { name: 'Метод формулы' });
  await waitFor(() => expect(method.disabled).toBe(true));
  expect(method.value).toBe('mica.charge22');
  expect(screen.queryByRole('option', { name: /Оливин/ })).toBeNull();
  ...
  expect(screen.getByRole('button', { name: 'Рассчитать этот анализ' }).disabled).toBe(true);
  await user.selectOptions(fe, 'all_fe2');
  await user.selectOptions(basis, 'ideal_O10_W2');

...
- 735 файлов Streamlit v1;
- 153 тестовых, regression, acceptance и smoke-скрипта;
...
Из 65 выбранных научных и операционных regression-проверок 58 прошли. Семь результатов требуют оговорки:
...
| `tests_edit_undo.py` | отсутствует | механизм есть в `petrolab/edit_undo.py`, но отдельного файла regression test нет |
| `tests_outliers.py` | отсутствует | модуль есть, но названного отдельного теста нет |
| browser screenshots | blocked | Streamlit запущен, но cloud browser не получил доступ к локальному порту; визуальный UX-аудит не заявляется завершённым |
...
| 13 | Workspace Snapshot: `navigation_state.py`, IgPet/ioGAS audit | отсутствует | Keep | восстанавливает Data Universe, Table View, панели, оси, видимость, Selection по явной политике и активный Calculation Run |
...
| 17 | Versioned mineral suggestion: `mineral_recognition.py`, 97 минералов/55 целей; alkaline extension | отсутствует | Keep | это suggestion, а не IMA-факт; ruleset, candidates, score/confidence и explicit acceptance обязательны |
| 18 | Formula registry: 13 mineral families, method IDs и oxygen bases: `minerals/formulae.py` | частично | Keep | каждый Derived Value хранит method id/version, basis, inputs, assumptions и независимый benchmark |
| 19 | Fe valence/Droop/OH и halogen guards: `formula_policy.py`, `amphibole_ima.py` | частично | Keep | неизвестная валентность не становится измеренной; OH и амфибольная классификация показывают предел применимости |
...
| 38 | Streamlit pages, wrappers, session keys и Quick/Advanced split | отсутствует | Reject | интерфейс и compatibility architecture не переносятся; v2 использует девять утверждённых экранов и progressive disclosure |
...
| Mineral recognition | 97 entries, 55 chemically resolvable targets, alkaline/carbonatite and oxide extensions | canonical positive cases, ambiguous cases, low-confidence unresolved cases, ruleset version |
